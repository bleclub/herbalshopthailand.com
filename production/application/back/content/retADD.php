...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {	
	if($_POST['content_id'] == ""){
		...
		$value_content = array(
			...
			"content_detail_th" => trim(@$_POST['content_detail_th']),
			"content_detail_en" => trim(@$_POST['content_detail_en']),
		...
		if ($query_content == TRUE) {
			/*Log file*/
				$value_log = array(
					...
					"log_detail" => 'เพิ่ม '.$_POST['content_categories'],
				...
				header("location:" . $baseUrl . "/back/content/list/".$_POST['content_categories']);	
		}
	}
	else {
		//$pwd = trim(salt_pass($_POST['password']));
		...
		if ($query_content == TRUE) {
			/*Log file*/
				$value_log = array(
					...
					"log_detail" => 'แก้ไข '.$_POST['content_categories'],
					"log_date" => date('Y-m-d H:i:s'),
					"log_ip" => $_SERVER["REMOTE_ADDR"],
					"log_user" => $_SESSION[_ss . 'username'] ,
					"log_user_id" => $_SESSION[_ss . 'id']
